                ...
                queryDetail.find({
                    success: function (results) {
                        ...
                        for (var i = 0; i < results.length; i++) {
                            var regUrl=results[i].get("website").match(/(https?)([\x00-\xff]+)/);


                            var toQueryUrl = 'http://api.page2images.com/restfullink?p2i_url=http' + regUrl[2] + '&p2i_key=ad65b1c619b4f20f&p2i_fullpage=1';
                            var item = {
                ...
                _.each(queryUrlList, function (oneQuery) {
                 async.waterfall([
                     ...
                     , function (error, response, body) {
                       if (error) {
                            ...
                            if (resBody.status == "processing") {
                                   ...
                                   console.log("Error response:  " + resBody + "  " + oneQuery.queryUrl);
                                   callback(null, 0, "", oneQuery.productDetailResult);
                                  }
                  ...
                  , function (flag, image_url, productDetailResult, callback) {
                       console.log("img_url's callback: " + flag + "   " + image_url + "   " + productDetailResult.get("website"));
                                    if (flag == 1) {
